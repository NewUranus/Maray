...
{
    /// <summary> This enum defines the windows messages we respond to. See more on Windows messages <a href="https://docs.microsoft.com/en-us/windows/win32/learnwin32/window-messages"> here </a> </summary>
    [SuppressMessage("ReSharper", "IdentifierTypo")]
    public enum WindowsMessages : uint
    {
        /// <summary>
        /// Notifies a window that the user clicked the right mouse button (right-clicked) in the window. See <a href="https://docs.microsoft.com/en-us/windows/win32/menurc/wm-contextmenu"> WM_CONTEXTMENU message </a>
        ///
        /// In case of a notify icon: If a user selects a notify icon's shortcut menu with the keyboard, the Shell now sends the associated application a WM_CONTEXTMENU message. Earlier versions send WM_RBUTTONDOWN and WM_RBUTTONUP messages. See <a href="https://docs.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shell_notifyiconw"> Shell_NotifyIcon function </a>
        /// </summary>
        ...
        /// <summary> Sent when the effective dots per inch (dpi) for a window has changed. The DPI is the scale factor for a window. There are multiple events that can cause the DPI to change. </summary>
        ...
        /// <summary> This message is only send when using NOTIFYICON_VERSION_4, the Shell now sends the associated application an NIN_SELECT notification. Send when a notify icon is activated with mouse or ENTER key. Earlier versions send WM_RBUTTONDOWN and WM_RBUTTONUP messages. </summary>
        NIN_SELECT = WM_USER,

        /// <summary> This message is only send when using NOTIFYICON_VERSION_4, the Shell now sends the associated application an NIN_SELECT notification. Send when a notify icon is activated with SPACEBAR or ENTER key. Earlier versions send WM_RBUTTONDOWN and WM_RBUTTONUP messages. </summary>
        ...
        /// <summary>
        /// Sent when the balloon disappears. For example, when the icon is deleted. This message is not sent if the balloon is dismissed because of a timeout or if the user clicks the mouse.
        ///
